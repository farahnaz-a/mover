...
class CommercialGoodsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $commercialgoods = CommercialGoods::create([
        ...
        if ($request->has('image')) {
            ...
            $filename  = $commercialgoods->id. '.' .$image->extension();
            $location  = public_path('uploads/commercialgoods/');
            $image->move($location , $filename);

            // Save Image name in the database
            $commercialgoods->image = $filename;
        }

        // Save Everything in database 
        $commercialgoods->save();
