...
class BoatsAndVoluminousController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $boatsvoluminous = BoatsAndVoluminous::create([
       ...
       if ($request->has('image')) {
            ...
            $filename  = $boatsvoluminous->id. '.' .$image->extension();
            $location  = public_path('uploads/boatsvoluminous/');
            $image->move($location , $filename);

            // Save Image name in the database
            $boatsvoluminous->image = $filename;
        }

        // Save Everything in database 
        $boatsvoluminous->save();
